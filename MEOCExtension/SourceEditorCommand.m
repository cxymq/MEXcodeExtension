...
- (void)performCommandWithInvocation:(XCSourceEditorCommandInvocation *)invocation completionHandler:(void (^)(NSError * _Nullable nilOrError))completionHandler {
    ...
    if ([invocation.commandIdentifier containsString:@"SourceEditorCommandSelectImportSort"]) {
        ...
        if (!defaults) {
            defaults = [[NSUserDefaults alloc] initWithSuiteName:@"group.com.MEXcodeExtension"];
        }
        NSData* configurationBookmark = [defaults dataForKey:@"clangFormatPathKey"];
        NSString *configurationPath = [[NSString alloc] initWithData:configurationBookmark encoding:NSUTF8StringEncoding];
//        NSData* configurationBookmark = [@"/Users/nazimai/Desktop/work/third lib/MEXcodeExtension/MEOCExtension/.clang-format" dataUsingEncoding:NSUTF8StringEncoding];
